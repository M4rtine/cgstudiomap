...
function open_partner_view(id) {
    // on_click event to open the view of the partner following
    // the given id.
    ...
    // this represents the current windows here.
    this.openerp.client.action_manager.do_action(action);
...
openerp.base_geoengine = function (openerp) {
     ...
     * a - {Object}
     */
    var divField = function (title, value) {
        return '<div><span class="content_fields_title">' + title + ' : </span><span class="content_field_value">' + value + '</span><br /></div>';
    };
    var divUrlField = function (title, value) {
        return '<div><span class="content_fields_title">' + title + ' : </span><span class="content_field_value"><a target="_blank" href="' + value + '">' + value + '</a></span><br /></div>';

    };
    var otherFields = function (a) {
        // Build the content of the main div of map_info
        var _t = openerp._t;
        var str = [];
        str.push('<div><span class="content_field_value">' + a['city'] + ', ' + a['country_id'][1] + '</span><br /></div>');
        return str.join('')
    };
    ...
    openerp.base_geoengine.formatHTML = function (a) {
        var str = [];
        var oid = a['id'];
        str.push('<div id="map_info_header"><span>' + a['name'] + '</span></div>');
        str.push('<div id="map_info_content">' + otherFields(a) + '</div>');
        str.push('<div id="map_info_footer"><button onclick="open_partner_view(' + oid + ') " string="View Form">more ...</button></div>')
        return str
    };
                ...
                case "osm":
                    out.push(
                        new OpenLayers.Layer.OSM(
                            l.name,
                            //New layer using http://developer.mapquest.com/web/products/open/map
    ...
    openerp.base_geoengine.GeoengineView = openerp.web.View.extend({
        ...
        view_loading: function (data) {
            ...
            this.$el.html(QWeb.render("GeoengineView", {
    ...
    openerp.base_geoengine.FieldGeoRect = openerp.web.form.AbstractField.extend({
        ...
        validate: function () {
            ...
            try {
                ...
                this.invalid = (this.required || !this.none_are_set(coords)) && !this.all_are_set(coords);
            } catch (e) {
                this.invalid = true;
            }
        },
